...
```
🌐 インターネット → 💻 EC2(Webアプリ) → 🗄️ RDS(MySQL)
                                            ↓
                                    📊 データ永続化
```
...
1. 作成した`employee-app-vpc`の詳細画面を開く

💡 **確認ポイント**: パブリックサブネットにはルートテーブルでインターネットゲートウェイへのルートがあるのに対して、プライベートサブネットにはルートテーブルでインターネットゲートウェイへのルートがないことを確認
...
#### インバウンドルール
- カスタムTCP (ポート3000): ソース Anywhere-IPv4（0.0.0.0/0） を追加

#### アウトバウンドルール
- 変更しないこと

### Step 3: DB用セキュリティグループ作成
...
- ルールを追加
  - **タイプ**: `MySQL/Aurora`
  - **ソース**: `web-server-sg`（🔍️に `web-server-sg` と入力して選択）
...
1. **RDSコンソール**→「フル設定で作成」→「作成」

![](images/create-rds-full-setting.png)
...
- **マスターユーザー名**: `admin`
- **認証情報管理**: `セルフマネージド`
- **マスターパスワード**: `password123`（本番では強力なパスワードを使用すべき。⚠️今回は演習であり、アプリケーションもこの値をハードコーディングしているので、 `password123` を使ってください。）
...
---
> 📌 **Day 4のハンズオンから遷移してきた方へ**: このPhase 2まで完了したら、[Day 4の手順](../../day4/ha-system-lab/README.md)へ戻ってください。

---

## 🚀 Phase 3: Webアプリケーション構築
...
- キーペアなしで続行（推奨されません）
（Session Manager で接続するため、キーペアは不要です）
...
3. アプリケーションの状態を確認
    - 正常に起動していれば、user-data 内でデータベース初期化も済んでいるため、Phase 4へ進む
    - 失敗している場合や、初期化が確認できない場合は、以下の手順 4〜6 で応急処置する
    ...
    **失敗している場合は、たいてい以下のどちらかに問題がある**  
    ...
    - セキュリティグループ（`database-sg`）の設定誤り → Phase 2のStep 3 を参考に設定しなおす  
...
- **マネージド**: OS・DB管理が不要
- **自動バックアップ**: データ保護が自動化
- **高可用性**: マルチAZ配置で障害対応(ただしサンドボックス環境の制限で、演習はシングルAZ構成)
...
明日（Day4）は、高可用性システムを構築します。今日作ったシステムを複数台構成にして、可用性を高めましょう！
