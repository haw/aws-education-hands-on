...
### **データベース**
- **RDS MySQL 8.4.9**: `employee-database-cf`
...
5. 「送信」をクリック
6. スタック一覧の画面に移動しますが、作成したスタック名（`Day3DbLabStack`）がすぐには表示されないことがあります。表示されない場合は**更新ボタン（🔄）**を押してください。

    ![](../images/stack-list-refresh.png)

### ✅ 完了状態の確認

- スタックのステータスが **`CREATE_COMPLETE`** になれば、AWSリソースの作成自体は完了です（所要時間の目安は下記「⏰ デプロイ時間」を参照）
- 「出力」タブに `ApplicationUrl` / `DatabaseEndpoint` / `VpcId` の3つが表示されていることを確認してください
- ⚠️ `CREATE_COMPLETE` になった直後は、EC2インスタンス内でアプリケーションのセットアップ（Node.jsのインストールやデータベースの初期化など）がまだ動いています。**数分待ってから** `ApplicationUrl` にアクセスしてください
- ブラウザで `ApplicationUrl` を開き、Employee Management Systemの画面が表示されれば完了です
